...
class Experiment:
    ...
    def load_pupil_data(self, isub):
        ...
        if not self.pupil_data_files:
            self.pupil_data_files = sorted(list(self.data_dir.glob("*pupildata*.mat")))

        pupil_data = sio.loadmat(self.pupil_data_files[isub])["pupildata"]
...
class Wrangler:
    ...
    def balance_labels(self, xdata, ydata, downsamp=None, return_cut=False):
        """
        balances number of class instances
        ...
        xdata_cut = np.delete(xdata, label_idx.astype(int), 0)
        ydata_cut = np.delete(ydata, label_idx.astype(int), 0)
        xdata = xdata[label_idx.astype(int), :, :]
        ydata = ydata[label_idx.astype(int)]

        if return_cut:
            return xdata, ydata, xdata_cut, ydata_cut
        else:
            return xdata, ydata
    ...
    def balance_data(self, X_train_all, X_test_all, y_train, y_test, minimize_loss=False):
        """
        ...
        y_train -- trial labels for training data
        y_test -- trial labels for testing data
        minimize_loss -- add cut test data to training data before balancing (default False)
        """
        if minimize_loss:
            # if minimizing loss, add cut test data to training data before balancing
            # Helpful when there is 1 more A than B in test, and 1 more B than A in train.
            # leads to balanced train and test, with more trials in train
            X_test_all, y_test, X_test_cut, y_test_cut = self.balance_labels(X_test_all, y_test, return_cut=True)
            X_train_all = np.concatenate((X_train_all, X_test_cut), 0)
            y_train = np.concatenate((y_train, y_test_cut), 0)
    ...
    def bin_and_split_data(self, xdata, ydata, test_size=0.20, train_labels=None):
        """
        ...
        for self.ifold in range(self.n_splits):
            ...
            if train_labels is not None:
                train_conds = [self.label_dict[k] for k in train_labels]
                test_conds = [self.label_dict[k] for k in self.labels if k not in train_labels]


                
                try:
                    X_train_opts, y_train_opts = self.select_labels(xdata_binned, ydata_binned, train_conds)
                    X_test_opts, y_test_opts = self.select_labels(xdata_binned, ydata_binned, test_conds)

                    X_train_all, X_test_trainConds, y_train, y_test_trainConds = train_test_split(
                        X_train_opts, y_train_opts, stratify=y_train_opts, test_size=test_size
                    )
                except:
                    print('Error in selecting training and testing conditions')
                    print('Train conditions:', np.unique(y_train_opts, return_counts=True))
                    print('Test conditions:', np.unique(y_test_opts, return_counts=True))
                    raise

                # testing on all conditions
                X_test_all = np.append(X_test_trainConds, X_test_opts, 0)  
                y_test = np.append(y_test_trainConds, y_test_opts, 0)
...
class Classification:
    ...
    def decode(self, X_train, X_test, y_train, y_test, y_test_shuffle, isub):
        ...
        for i, ss in enumerate(set(y_test_shuffle)):
            self.confidence_scores_shuff[isub, itime, ifold, i] = np.mean(confidence_scores[y_test_shuffle == ss])
...
class Interpreter:
    def __init__(self, clfr=None, subtitle="", output_dir=None, experiment_name=""):
        ...
        if clfr is not None:
            ...
            self.confidence_scores_shuff = clfr.confidence_scores_shuff
    ...
    def plot_ss(
        ...
        for iss, ss in enumerate(condition_subset):
            ...
            if palette is not None:
                color_kwargs = {'color': palette[cond]}
            else:
                color_kwargs = {}
            ...
            # ERP
            ax.plot(self.info["times"], x, linewidth=1.5, alpha=0.8, label=cond, **color_kwargs)
            # SE
            ax.fill_between(self.info["times"], x - se, x + se, alpha=0.3, **color_kwargs)
